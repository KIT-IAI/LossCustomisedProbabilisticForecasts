...
def train_forecast_pipeline(pipeline_name, target, deepar, calendar_extraction, features):
    pipeline = Pipeline(f"../../Results/{pipeline_name}/train_deepar", name="train_deepar")
...
def get_prob_forecast_pipeline(pipeline_name, target, calendar_extraction, target_scaler, deepar, features,
                               feature1_scaler, feature2_scaler, feature3_scaler, feature4_scaler):
    pipeline = Pipeline(f"../../Results/{pipeline_name}/evaluate_deepar", name=f"evaluate_deepar")
    ...
    if number_of_features == 0:
        prob_forecast = deepar(x=target_slice,
                               calendar=calendar_slice,
                               computation_mode=ComputationMode.Transform)

        scaled_results_dict[deepar.name] = prob_forecast
        ...
        final_results_dict[deepar.name] = rescaled_forecast
